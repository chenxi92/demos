...
extension View {
    @ViewBuilder
    func wrappedInNavigationViewToMakeDismisable(_ dismiss: (() -> Void)?) -> some View {
...
extension View {
    func compactableToolbar<Content>(@ViewBuilder content: () -> Content) -> some View  where Content: View {
        self.toolbar {
            content().modifier(CompactableInfoContextMenu())
        }
    }
}

struct CompactableInfoContextMenu: ViewModifier {
